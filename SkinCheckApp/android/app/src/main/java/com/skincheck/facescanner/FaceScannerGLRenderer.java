...
//      capturePhoto()/sampleFaceRegions(), which read the latest state
...
public class FaceScannerGLRenderer implements GLSurfaceView.Renderer {
  ...
  public static class FaceScannerState {
    ...
    // Confirmed on a real device: yaw was inverted (turning left was being
    // reported as positive yaw, matching the "right" target instead of
    // "left"). Flipped from 1f based on that feedback. This single
    // constant drives both alignment detection AND the on-screen arrow
    // direction in ScanScreen.tsx, so this one fix corrects both at once.
