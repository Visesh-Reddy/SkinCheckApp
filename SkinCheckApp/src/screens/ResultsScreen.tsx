...
import { zoneTip, ageTip } from '../logic/tips';
import { computeProgress, getAllScanRecords, ProgressComparison } from '../native/scanHistory';
import { getPhotoBase64 } from '../native/FaceScannerModule';
...
export function ResultsScreen({ zoneStats, age, photoUri, onScanAgain }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!photoUri) return;
    // Photos are encrypted at rest -- decrypt in memory for display only,
    // never write the decrypted bytes back to disk.
    getPhotoBase64(photoUri)
      .then((base64) => {
        if (!cancelled) setPhotoDataUri(`data:image/jpeg;base64,${base64}`);
      })
      .catch(() => {
  ...
  const suggestions = useMemo(() => {
    if (!hasData) return [];
    const zoneSuggestions = scored
      .filter((z) => z.issue !== 'healthy')
      .sort((a, b) => a.score - b.score)
      .slice(0, 3)
      .map((z) => zoneTip(ZONE_LABELS[z.key] ?? z.key, z.issue))
      .filter((t): t is string => t !== null);
    const age_ = ageTip(age);
    return age_ ? [...zoneSuggestions, age_] : zoneSuggestions;
  }, [scored, age, hasData]);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.photoCard}>
        {photoDataUri ? (
          <Image source={{ uri: photoDataUri }} style={styles.photo} resizeMode="cover" />
        ) : (
          <View style={[styles.photo, styles.photoPlaceholder]}>
            <Text style={styles.photoPlaceholderText}>Decrypting photo...</Text>
          </View>
        )}
      </View>

      <View style={[styles.overallCard, { borderColor: overallColor.text + '55' }]}>
...
const styles = StyleSheet.create({
  ...
  emptyBody: { color: colors.textSecondary, textAlign: 'center', fontSize: 13.5, lineHeight: 20, marginBottom: spacing.sm },
  ...
  tipRow: { flexDirection: 'row', gap: spacing.sm, alignItems: 'flex-start', paddingVertical: 8 },
  tipDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.accentStart, marginTop: 6, flexShrink: 0 },
