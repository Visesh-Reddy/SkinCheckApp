...
//
// IMPORTANT: these directions encode the same yaw sign assumption as
// POSE_TARGETS in poseTargets.ts (which itself mirrors YAW_SIGN in
// FaceScannerGLRenderer.java -- see that file's calibration note). If
// real-device testing ever requires flipping that sign, the arrows here
...
const STEP_ARROW: Record<ScanStepKey, { symbol: string; position: 'left' | 'right' | null }> = {
...
function DirectionArrow({ step }: { step: ScanStepKey }) {
  ...
  const positionStyle = arrow.position === 'left' ? styles.arrowLeft : styles.arrowRight;
