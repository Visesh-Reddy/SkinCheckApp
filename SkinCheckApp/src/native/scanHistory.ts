...
export async function getAllScanRecords(): Promise<ScanRecord[]> {
  ...
  try {
    // The library's documented usage checks for `undefined` on a missing
    // key (i.e. it resolves rather than throwing) -- this try/catch is
    // defensive redundancy in case that differs on some platform/version,
    // not because throwing is confirmed behavior.
